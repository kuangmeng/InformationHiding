...
for i = 1 : 10
    ...
    for j = 1 : m
        ...
        for k = 1 : n
            cv = [(k - 1) * 8 + 1 : k * 8];
            tmp = s(rv, cv);
            cor(1) = SpaceCor(tmp);% 不进行任何翻转，直接求图像块空间相关性
            cor(2) = SpaceCor(f1(tmp, M));% 经过非负翻转图像块空间相关性
            cor(3) = SpaceCor(f_1(tmp, M));% 经过非正翻转图像块空间相关性
            if cor(2) > cor(1)
            %计算 RS
...
rs = rs / (m * n);
figure;
plot([0.1:0.1:1.0], rs(:, 1),[0.1:0.1:1.0],rs(:, 2),[0.1:0.1:1.0],rs(:, 3),[0.1:0.1:1.0],rs(:, 4)),title('图片相关参数');
